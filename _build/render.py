...
<!-- SOCIAL METADATA. Added 2026-08-18. This site had NONE — no og:title, no
     ...
     og:image was deliberately ABSENT rather than pointed at a placeholder: a
     tag naming a file that 404s is worse than no tag, because a scraper
     records the miss. The card now exists, so the tag does. 2026-09-02:
     /og.png, 1200x630, drawn from the LIVE computed palette rather than a
     guess -- #0057B8 read off the page's own link colour, #1D1D1F off its
     body text, Space Grotesk off its h1 -- and carrying this site's own
     headline. The motif is a point estimate inside its interval, which is
     the product's whole claim rather than decoration.

     twitter:card moves from summary to summary_large_image with it: a 1.91:1
     card served as `summary` renders as a thumbnail and wastes the image. -->
...
<meta property="og:image" content="https://bookbreaker.bet/og.png">
<meta property="og:image:width" content="1200">
<meta property="og:image:height" content="630">
<meta property="og:image:alt" content="Bookbreaker — find your edge, with the error bar.">
<meta name="twitter:card" content="summary_large_image">
<meta name="twitter:image" content="https://bookbreaker.bet/og.png">
...
STYLE = """/* PASS 16 — a typeface,because we were the only one of the six without one.
   ...
   Measured: langchain sets its headline in Twklausanne,cofounder in neoris,framer in GT Walsheim,avo in Inter. Ours resolved to -apple-system,which
...
.rf,table,.figure,.screen,.plate{font-variant-numeric:tabular-nums lining}
...
body.home main>.plate,body.home main>.scroll,body.home main>.figure,body.home main>.screen{max-width:var(--measure-plate)}
...
.wall{max-width:var(--measure-plate);
...
/* the value Bookbreaker actually stands behind: solid{position:absolute;left:var(--rl);bottom:-5px;
...
.rf--hero .rf-pt{top:26%;height:48%;width:2px;margin-left:-1px}
...
/* Marks a competitor's over-precision. The dotted rule is the whole device:
   dimming just the decimals would mean wrapping them in a tag,neither of which is a measured number,and the build fails. The number stays
   one text node. */
.over{border-bottom:1px dotted var(--rule)}

/* ---------- the terminal block: the one dark object{
...
/* tabular figures make + and - exactly one digit wide{text-align:right}
...
/* fill stays deliberately subordinate: a bar with NO terminators{display:flex;align-items:center;gap:var(--s-2);min-width:0}
...
/* the band is now the most valuable column on the strip{grid-template-columns:1.4fr .62fr .78fr 1.6fr .9fr .85fr}
...
/* Lifted and turned a degree off true{margin:0;box-shadow:
...
/* Realised EV is the number the product exists to show{color:var(--good);font-weight:650}
...
/* The matchup column was narrow enough to wrap "DEN @ PHX" onto two lines{grid-template-columns:1.75fr .95fr .62fr .72fr 1.85fr .5fr .6fr .85fr;
  gap:var(--s-2)}
...
/* ------------------------------------------------------------ data plates
   ...
   the build went GREEN anyway: rf() puts geometry in a style="" attribute,so an
...
.trust>div,.trust>li,.cards .card,.cards>li,figure.plate,.demo-box{
  position:relative;
  background:
    linear-gradient(180deg,
      color-mix(in srgb, var(--ink) 4%, var(--card)) 0%,
      var(--card) 45%)}
.trust>div::before,.cards .card::before,figure.plate::before{
...
/* PASS 12 — the hero depicts the window that ships.
   New class names throughout (.win*){
...
/* PASS 14 — the page had one plane, and Outlier has three.

   Measured before writing this: every top-level section on the homepage
   computed to background rgba(0,0,0,0). Content sat directly on the plate
   ...
   It also fixes the spacing, which is why it is one pass and not two. There
   were three rhythms competing: .wall and .demo pushed 86.4px margins,
   .pitch used an 88px grid gap, and .speed and .heat-home used 86.4px
   padding with no margin at all — so the gap between two sections was 86px
   or 0px depending on which pair you looked at, and the content blocks
   inside them ran from 67px to 161px apart. A surface with its own padding
   makes the outer gap one number, because the breathing room moved inside
   the box where it belongs. */
.wall,.demo,.speed,.heat-home,.hp,.vs-side,.pitch>article{
  background:var(--card);
  border:1px solid var(--rule);
  border-radius:14px;
...
:root{
  --float-pad:clamp(1.5rem,3vw,2.5rem);
  --float-gap:clamp(2.25rem,4vw,3.5rem);
}
/* One rhythm. Margin on one side only, so two adjacent floats cannot
   collapse to a different gap than a float beside a plain block. */
.wall,.demo,.speed,.heat-home,.close,.pitch{
  margin:0 0 var(--float-gap);
  padding-block:var(--float-pad);
}
.pitch{background:none;border:0;padding-inline:0;box-shadow:none}
/* .pitch is display:block, not a grid — zeroing the article margin stacked
   the cards edge to edge. They carry the rhythm themselves. */
.pitch>article{margin:0 0 var(--float-gap)}
.pitch>article:last-child{margin-bottom:0}
main>.trust{margin-bottom:var(--float-gap)}

/* The lift. Outlier gets it almost entirely from tone, so this is
   deliberately small — enough that the edge reads as an edge, not enough to
   look like a 2013 card. On light it is a soft grey; on dark, shadow is
   nearly invisible against a near-black ground, so the top hairline does the
   work instead. */
.wall,.demo,.speed,.heat-home,.hp,.vs-side,.pitch>article,.close{
  box-shadow:0 1px 2px rgba(16,24,40,.04),0 8px 24px -12px rgba(16,24,40,.10);
}
@media (prefers-color-scheme:dark){
  :root:not([data-theme="light"]) .wall,
  :root:not([data-theme="light"]) .demo,
  :root:not([data-theme="light"]) .speed,
  :root:not([data-theme="light"]) .heat-home,
  :root:not([data-theme="light"]) .hp,
  :root:not([data-theme="light"]) .vs-side,
  :root:not([data-theme="light"]) .pitch>article,
...
@media (prefers-reduced-motion:reduce){
  .wall,.demo,.speed,.heat-home,.hp,.vs-side,.pitch>article,.close,.own,
  .trust>div,.trust>li,ul.states li,figure.plate,.btn,.cta a,.btn.primary,
...
#: The three selectors a palette generation is written against. Anything that
#: sets a token elsewhere is a component override and is left alone.
_ROOT_CONTEXTS = (
    ':root',
    ':root:not([data-theme="light"])',
    ':root[data-theme="dark"]',
...
def _root_blocks(css):
    """Every :root-family block, as (selector, start, end) over the body.

    Matched wherever the selector appears, not only at the start of a line:
    the dark blocks are written `@media (...){:root:not(...){` on one line,
    and a line-anchored version of this could not see them. It found them
    before consolidation and not after, so the check comparing the two
    reported 24 tokens deleted that were in fact still there. The instrument
    could not read its own output.
    """
    out = []
    for sel in _ROOT_CONTEXTS:
        for m in re.finditer(re.escape(sel) + r"\s*\{", css):
            # `:root` also matches inside `:root[data-theme="dark"]`; require
            # the brace to belong to this selector, not a longer one.
            if sel == ":root" and css[m.start() - 1:m.start()] not in ("", "\n", "}", " ", "{"):
                continue
            end = css.find("}", m.end())
            if end >= 0:
                out.append((sel, m.end(), end))
    return sorted(out, key=lambda b: b[1])


def resolve_palette(css):
    """What each token actually evaluates to, per context, after the cascade.

    This is the thing that must not change. Every check below compares this
    map before and after, so a consolidation that alters one colour fails
    rather than shipping.
    """
    resolved = {}
    for sel, lo, hi in _root_blocks(css):
        for m in re.finditer(r"(--[a-z0-9-]+)\s*:\s*([^;]+);", css[lo:hi]):
            resolved[(sel, m.group(1))] = m.group(2).strip()
    return resolved


def canonicalise_palette(css):
    """Fold every palette generation into one block per context.

    Three complete palettes accumulated in this stylesheet — sky, blue, and
    FanDuel — because a repalette was written as an append and the cascade
    quietly did the right thing. It cost real time twice: a `theme-color`
    gate passed on the exact bug it was written for, because the superseded
    hex was still present as a dead override and the gate only asked whether
    the hex appeared *anywhere*.

    Nothing here changes what renders. The last declaration already won; this
    deletes the ones that had already lost, so that "present in the
    stylesheet" and "actually in use" stop being different questions.
    """
    before = resolve_palette(css)

    # Strip token declarations from every :root block, back to front so the
    ...
    light = {t: v for (sel, t), v in before.items() if sel == ":root"}
    dark_media = {t: v for (sel, t), v in before.items()
                  if sel == ':root:not([data-theme="light"])'}
    dark_class = {t: v for (sel, t), v in before.items()
                  if sel == ':root[data-theme="dark"]'}

    def block(tokens):
        return "".join(f"  {t}:{v};\n" for t, v in sorted(tokens.items()))

    canon = (
        "/* The palette, once. Generated by canonicalise_palette() — every\n"
        "   token is declared exactly one time per context, so the value in\n"
        "   this file is the value that renders. Appending an override\n"
        "   elsewhere still works, and gets folded in here on the next\n"
        "   build; check_one_palette() fails if one is left loose. */\n"
        ":root{\n" + block(light) + "}\n"
        "@media (prefers-color-scheme:dark){\n:root:not([data-theme=\"light\"]){\n"
        + block(dark_media) + "}}\n"
        ':root[data-theme="dark"]{\n' + block(dark_class) + "}\n"
    ...
    after = resolve_palette(css)
    if after != before:
        changed = {k: (before.get(k), after.get(k))
                   for k in set(before) | set(after) if before.get(k) != after.get(k)}
        raise SystemExit(f"palette consolidation changed {len(changed)} values: "
                         f"{list(changed.items())[:4]}")
    return css


STYLE = canonicalise_palette(STYLE)
